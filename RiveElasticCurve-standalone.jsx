...
function ElasticCurveView(viewModel) {
    ...
    this.build = function() {
        ...
        // 曲线预览区域
        var previewGroup = _window.add('group');
        previewGroup.orientation = 'column';
        previewGroup.alignChildren = ['fill', 'top'];
        
        var previewLabel = previewGroup.add('statictext', undefined, '曲线预览:');
        previewLabel.graphics.font = ScriptUI.newFont('dialog', 'BOLD', 11);
        
        // 创建自定义绘图面板
        _controls.curveCanvas = previewGroup.add('panel', undefined, '');
        _controls.curveCanvas.preferredSize = [280, 150];
        _controls.curveCanvas.alignment = ['fill', 'top'];
        
        // 绘制曲线的函数
        _controls.curveCanvas.onDraw = function() {
            var g = this.graphics;
            ...
            // 背景
            g.backgroundColor = g.newBrush(g.BrushType.SOLID_COLOR, [0.15, 0.15, 0.15]);
            g.rectPath(0, 0, w, h);
            g.fillPath();
            
            // 网格线
            g.newPath();
            var gridPen = g.newPen(g.PenType.SOLID_COLOR, [0.25, 0.25, 0.25], 1);
            for (var i = 0; i <= 4; i++) {
                var x = padding + (graphW / 4) * i;
                var y = padding + (graphH / 4) * i;
                g.moveTo(x, padding);
                g.lineTo(x, padding + graphH);
                g.moveTo(padding, y);
                g.lineTo(padding + graphW, y);
            }
            g.strokePath(gridPen);
            
            // 获取当前参数
            var amplitude = parseFloat(_controls.amplitudeInput.text) || 1.0;
            var period = parseFloat(_controls.periodInput.text) || 0.5;
            var easingType = ['easeOut', 'easeIn', 'easeInOut'][_controls.easingDropdown.selection.index];
            
            // 绘制曲线
            g.newPath();
            var curvePen = g.newPen(g.PenType.SOLID_COLOR, [0.3, 0.7, 1.0], 2);
            
            var steps = 100;
            for (var i = 0; i <= steps; i++) {
                var t = i / steps;
                var value = calculateElasticValue(t, amplitude, period, easingType);
                
                // 将值映射到画布坐标（翻转 Y 轴，因为屏幕坐标向下为正）
                var x = padding + t * graphW;
                var y = padding + graphH - (value * graphH);
                
                // 限制 Y 在可见范围内
                y = Math.max(padding, Math.min(padding + graphH, y));
            ...
            // 绘制起点和终点标记
            var markerPen = g.newPen(g.PenType.SOLID_COLOR, [1.0, 0.3, 0.3], 1);
            var markerBrush = g.newBrush(g.BrushType.SOLID_COLOR, [1.0, 0.3, 0.3]);
            
            // 起点
            g.ellipsePath(padding - 3, padding + graphH - 3, 6, 6);
            g.fillPath(markerBrush);
            
            // 终点
            g.ellipsePath(padding + graphW - 3, padding - 3, 6, 6);
            g.fillPath(markerBrush);
        };
        
        // 弹性曲线计算函数（简化版，用于预览）
        function calculateElasticValue(t, amplitude, period, easingType) {
            if (t === 0) return 0;
            if (t === 1) return 1;
    ...
    this._bindEvents = function() {
        ...
        _controls.amplitudeInput.onChange = function() {
            ...
            if (!result.success) {
                ...
                // 更新曲线预览
                _controls.curveCanvas.notify('onDraw');
            }
        };
    ...
    this.show = function() {
        if (_window) {
            ...
            // 如果窗口位置仍然不合理，强制设置到屏幕中心
            var screenBounds = _window.windowBounds;
            if (screenBounds.left < 0 || screenBounds.top < 0) {
                // 获取屏幕尺寸（粗略估算）
                var screenWidth = 1920;  // 默认值
                var screenHeight = 1080; // 默认值
                
                // 计算窗口尺寸
                var winWidth = _window.size.width;
                var winHeight = _window.size.height;
            ...
            // palette 窗口使用 show() 不会阻塞，可以继续操作 AE
            _window.show();
        }
    };
...
function init() {
    try {
        ...
        if (aeVersion < 11.0) {
            alert('此插件需要 After Effects CS6 或更高版本\n当前版本: ' + app.version);
            return;
        }
